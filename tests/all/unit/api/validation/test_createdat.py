...
class TestCreatedatValidation(OpenEventTestCase):
    def test_createdat(self):
        ...
        with self.app.test_request_context():
            ...
            test_event = Event(
                name='test event', starts_at=datetime.now(),
                ends_at=datetime.now()+timedelta(days=1),
                logo_url='foo.com')
            test_session = Session(title='Test Session')           
            test_access_code = AccessCode(code=123)
            test_order = Order()
            test_ticket_holder = TicketHolder(
                firstname="John",
                lastname="Snow")
            test_role_invite = RoleInvite(
                email="testrole@mail.com",
                role_name="Nothing")
            test_discount_code = DiscountCode(
                code="123", value=0.0,
                type="test", used_for="nothing")
            test_event_invoice = EventInvoice()
            test_models = [
                test_user, test_event, test_access_code, test_order,
                test_ticket_holder, test_role_invite, test_discount_code,
                test_session, test_event_invoice]
            model_classes = [
                User, Event, AccessCode, 
                Order, TicketHolder, RoleInvite,
                DiscountCode, Session, EventInvoice]
            test_models_with_utcnow = [
                test_ticket_holder, test_discount_code,
                test_event_invoice]
            ...
            for model, model_class in zip(test_models, model_classes):
                with self.subTest(model=model, model_class=model_class):
                    save_to_db(model)
                    model_id = model.id
                    model_createdat = db.session \
                        .query(model_class).get(model_id).created_at
