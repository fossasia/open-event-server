...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ['event_topic_id'], ['event_topics.id'], ondelete='CASCADE'
        ),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('id'),
    )
    op.execute(
        "CREATE UNIQUE INDEX slug_event_topic_id ON event_sub_topics (slug, event_topic_id)"
    )
    ...
    op.execute(
        'INSERT INTO event_sub_topics(name, slug, event_topic_id) SELECT DISTINCT event_sub_topic_id, lower(replace(regexp_replace(event_sub_topic_id, \'& |,\', \'\', \'g\'), \' \', \'-\')), event_topic_id\
    ...
    op.execute(
        'ALTER TABLE events ALTER COLUMN event_sub_topic_id TYPE integer USING event_sub_topic_id::integer'
    )
    ...
    op.execute(
        'UPDATE event_types set slug=replace(slug, \'/\', \'-\') where slug like \'%/%\''
    )
    op.execute(
        'UPDATE event_topics set slug=replace(slug, \'/\', \'-\') where slug like \'%/%\''
    )
    op.execute(
        'UPDATE event_sub_topics set slug=replace(slug, \'/\', \'-\') where slug like \'%/%\''
    )
...
def downgrade():
    ...
    op.execute(
        'ALTER TABLE events ALTER COLUMN event_sub_topic_id TYPE varchar USING event_sub_topic_id::varchar'
    )
    op.execute(
