...
def upgrade():
    ...
    op.alter_column(
        u'permissions', 'service_id', existing_type=sa.INTEGER(), nullable=True
    )
    op.create_unique_constraint(
        'role_service_uc', 'permissions', ['role_id', 'service_id']
    )
    ...
    op.alter_column(
        u'role', 'name', existing_type=sa.VARCHAR(length=128), nullable=False
    )
...
def downgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.add_column(
        u'user', sa.Column('role', sa.VARCHAR(), autoincrement=False, nullable=True)
    )
    op.drop_constraint(None, 'role', type_='unique')
    op.alter_column(
        u'role', 'name', existing_type=sa.VARCHAR(length=128), nullable=True
    )
    op.add_column(
        u'permissions',
    ...
    op.add_column(
        u'permissions',
        sa.Column('modes', sa.INTEGER(), autoincrement=False, nullable=False),
    )
    op.add_column(
        u'permissions',
    ...
    op.create_foreign_key(
        u'permissions_user_id_fkey', 'permissions', 'user', ['user_id'], ['id']
    )
    op.create_unique_constraint(
        u'user_service_uc', 'permissions', ['user_id', 'service', 'service_id']
    )
    op.drop_constraint('role_service_uc', 'permissions', type_='unique')
    op.alter_column(
        u'permissions', 'service_id', existing_type=sa.INTEGER(), nullable=False
    )
