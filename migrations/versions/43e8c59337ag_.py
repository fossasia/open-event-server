...
def upgrade():
    op.execute(
        "INSERT INTO permissions(role_id, service_id, can_create, can_read, can_update, can_delete) \
...
def downgrade():
    op.execute(
        "DELETE FROM permissions WHERE role_id=(SELECT id FROM roles WHERE name='owner')"
    )
