...
class DataGetter(object):
    ...
    def get_live_events():
        return Event.query.join(Event.roles, aliased=True).filter_by(user_id=login.current_user.id) \
            ...
            .filter(Event.state == 'Published').filter_by(in_trash=False)

    @staticmethod
    def get_draft_events():
        return Event.query.join(Event.roles, aliased=True).filter_by(user_id=login.current_user.id) \
            .filter(Event.state == 'Draft').filter_by(in_trash=False)

    @staticmethod
    def get_past_events():
        return Event.query.join(Event.roles, aliased=True).filter_by(user_id=login.current_user.id) \
            .filter(Event.end_time <= datetime.datetime.now()).filter(
            or_(Event.state == 'Completed', Event.state == 'Published')).filter_by(in_trash=False)
    ...
    def get_live_and_public_events():
        return DataGetter.get_all_live_events().filter(Event.privacy != 'private').filter_by(in_trash=False)

    @staticmethod
    def get_all_draft_events():
        return Event.query.filter(Event.state == 'Draft').filter_by(in_trash=False)
    ...
    def get_all_past_events():
        return Event.query.filter(Event.end_time <= datetime.datetime.now()).filter(
            ...
            Event.in_trash is not True).filter_by(in_trash=False)
