...
class TicketingView(BaseView):
    ...
    def view_order(self, invoice_identifier):
        order = TicketingManager.get_and_set_expiry(invoice_identifier)
        if not order or order.status == 'expired':
            abort(404)
        if order.status == 'completed':
            return redirect(url_for('ticketing.view_order_after_payment', order_identifier=invoice_identifier))
