...
def event_invoices(invoice_identifier):
    if not current_user:
        raise ForbiddenException({'source': ''}, 'Authentication Required to access Invoice')
    try:
        event_invoice = EventInvoice.query.filter_by(identifier=invoice_identifier).first()
        event_id = event_invoice.event_id
    except NoResultFound:
        raise NotFoundException({'source': ''}, 'Event Invoice not found')
...
def order_invoices(order_identifier):
    if current_user:
        try:
            ...
            raise NotFoundException({'source': ''}, 'Order Invoice not found')
