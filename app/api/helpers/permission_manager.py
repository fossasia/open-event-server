...
def is_super_admin(view, view_args, view_kwargs, *args, **kwargs):
    ...
    if not user.is_super_admin:
        return ForbiddenException({'source': ''}, 'Super admin access is required')
    return view(*view_args, **view_kwargs)
...
def is_speaker_for_session(view, view_args, view_kwargs, *args, **kwargs):
    ...
    try:
        ...
        raise NotFoundException({'parameter': 'id'}, 'Session not found.')
...
def is_speaker_itself_or_admin(view, view_args, view_kwargs, *args, **kwargs):
    ...
    raise ForbiddenException({'source': ''}, 'Detail ownership is required, access denied.')
...
def is_session_self_submitted(view, view_args, view_kwargs, *args, **kwargs):
    ...
    try:
        ...
        raise NotFoundException({'parameter': 'session_id'}, 'Session not found.')
...
def is_registrar(view, view_args, view_kwargs, *args, **kwargs):
    ...
    if user.is_registrar(event_id) or user.has_event_access(event_id):
        return view(*view_args, **view_kwargs)
    raise ForbiddenException({'source': ''}, 'Registrar Access is Required.')
...
def is_moderator(view, view_args, view_kwargs, *args, **kwargs):
    ...
    raise ForbiddenException({'source': ''}, 'Moderator Access is Required.')
...
def permission_manager(view, view_args, view_kwargs, *args, **kwargs):
    ...
    if 'order_identifier' in view_kwargs:
        try:
            ...
            raise NotFoundException({'parameter': 'order_identifier'}, 'Order not found')
    ...
    if 'event_identifier' in view_kwargs:
        try:
            ...
            raise NotFoundException({'parameter': 'event_identifier'}, 'Event not found')
    ...
    if view_kwargs.get('event_invoice_identifier') is not None:
        try:
            ...
            raise NotFoundException({'parameter': 'event_invoice_identifier'}, 'Event Invoice not found')
    ...
    if 'identifier' in view_kwargs:
        try:
            ...
            raise NotFoundException({'parameter': 'identifier'}, 'Event not found')
    ...
    if 'fetch' in kwargs:
        ...
        if not fetched:
            ...
            if not found:
                raise NotFoundException({'source': ''}, 'Object not found.')
