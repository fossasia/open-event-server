...
def export_sessions_csv(sessions):
    ...
    for session in sessions:
        if not session.deleted_at:
            column = [
                session.title + ' (' + session.state + ')' if session.title else ''
            ]
            if session.speakers:
                in_session = ''
...
def export_speakers_csv(speakers):
    ...
    for speaker in speakers:
        ...
        column.append(
            speaker.speaking_experience if speaker.speaking_experience else ''
        )
        column.append(
