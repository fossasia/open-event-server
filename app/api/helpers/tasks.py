...
def resize_speaker_images_task(self, speaker_id, photo_url):
    ...
    try:
        ...
        uploaded_images = create_save_image_sizes(
            photo_url, 'speaker-image', speaker_id
        )
...
def export_xcal_task(self, event_id, temp=True):
    ...
    try:
        ...
        if temp:
            xcal_url = upload(
                xcal_file,
                UPLOAD_PATHS['exports-temp']['xcal'].format(event_id=event_id),
            )
        else:
            xcal_url = upload(
...
def export_order_csv_task(self, event_id):
    ...
    try:
        filedir = os.path.join(
            current_app.config.get('BASE_DIR'), 'static/uploads/temp/'
        )
        if not os.path.isdir(filedir):
            os.makedirs(filedir)
        ...
        order_csv_url = upload(
            order_csv_file,
            UPLOAD_PATHS['exports-temp']['csv'].format(
...
def export_order_pdf_task(self, event_id):
    ...
    try:
        order_pdf_url = create_save_pdf(
            ...
            UPLOAD_PATHS['exports-temp']['pdf'].format(
                event_id=event_id, identifier=''
            ),
        )
