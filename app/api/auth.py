...
def authorized_access_ticket(ticket_function):
    ...
    def wrapper(*args, **kwargs):
        ticket_holder = TicketHolder.query.filter_by(email=current_user.email).first()
        if ticket_holder is None:
            return NotFoundError({'source': ''}, 'This ticket is not associated with any ticket holder').respond()
        if not current_identity.is_anonymous and current_user.id == TicketHolder.user.id:
