...
from app.api.helpers.mail import send_email_with_action, send_email_confirmation
...
from app.models.mail import PASSWORD_RESET, PASSWORD_CHANGE, PASSWORD_RESET_AND_VERIFY
...
def authenticate(allow_refresh_token=False, existing_identity=None):
    ...
    access_token = create_access_token(
        identity.id, fresh=True, expires_delta=expiry_time
    )
        ...
        if get_count(db.session.query(User).filter_by(email=user_details['email'])) > 0:
            ...
            return make_response(
                jsonify(
                    user_id=user.id,
                    email=user.email,
                    oauth_hash=user.facebook_login_hash,
                ),
                200,
            )
