...
class OrderDetail(ResourceDetail):
    ...
    def before_update_object(self, order, data, view_kwargs):
        ...
        if (
            has_access('is_organizer', event_id=order.event_id)
            and 'order_notes' in data
        ):
...
def alipay_return_uri(order_identifier):
    ...
    try:
        ...
        if charge_response.status == 'succeeded':
            ...
            save_to_db(order)
            return redirect(
                make_frontend_url('/orders/{}/view'.format(order_identifier))
            )
        else:
