...
var app = new Vue({
    ...
    computed: {
        ...
        invalidDateRange: function () {
            if(this.event.ends_at_date.trim().length <= 0
                || this.event.ends_at_time.trim().length <= 0
            ...
            var end = moment(this.event.ends_at_date.trim() + " " + this.event.ends_at_time.trim(), format);
    ...
    watch: {
        ...
        'event.ends_at_time': function () {
            this.disableMove = shouldDisableMove(this);
        },
        'event.ends_at_date': function () {
            this.disableMove = shouldDisableMove(this);
        },
        'event.tickets': {
            handler :  function () {
    ...
    methods: {
        ...
        addTicket: function (ticketType) {
            ...
            ticket.sales_ends_at = moment().tz(this.event.timezone).add(10, 'days').hour(22).minute(0).format('HH:mm');
            ticket.price = 0.01;
...
function shouldDisableMove($this) {
    return (
        ...
        $this.event.ends_at_time.trim() ==='' ||
        $this.event.ends_at_date.trim() === '' ||
